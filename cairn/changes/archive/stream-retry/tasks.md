...
- [x] Add `Retry` and `DEFAULT_TIMEOUT` to the stream module.
...
- [ ] Follow-up, outside this repository: io-imap's mailbox watcher and io-jmap's watch loop select `Retry::Never` when they are bumped, their `WouldBlock` wakeup being a shutdown poll rather than a failure.
