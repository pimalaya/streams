...
The type kept its name after a look at `StreamProxy`. `Proxy` sits beside `Tls` as a configuration vocabulary in its own right, and the crate name plus the module path already say whose proxy it is. The rest of the crate followed the same reasoning shortly after, in [drop-prefix](./2026-08-15-drop-prefix.md).
