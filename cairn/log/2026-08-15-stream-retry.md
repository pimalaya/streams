...
A stream reporting `EAGAIN` used to end whatever exchange was in flight above it. It now carries a `Retry` strategy that `Read`, `Write` and `flush` honor: `Until(Duration)` retries a not-ready stream until that long passes without progress, `Never` hands the failure back to a caller whose loop wants it. Streams open on `Until` one minute.
...
One follow-up leaves this repository: io-imap's mailbox watcher and io-jmap's watch loop arm a five second deadline to poll a shutdown flag, and select `Retry::Never` when they are bumped, or their wakeup gets absorbed by the retry.
