...
//! feature) is the blocking transport: one `Read + Write` handle over
//! TCP, Unix sockets or a TLS session, with the plain-to-TLS upgrade
//! STARTTLS flows need. The module is deliberately named after its
//! runtime: a future async runtime would gain a sibling module (tokio)
//! next to it.
//!
//! ## Conventions
//!
