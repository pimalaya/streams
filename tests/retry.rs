...
use pimalaya_stream::{
    retry::Retry,
    stream::{Stream, UnixConnectOptions},
};
...
fn connected(name: &str, retry: Retry) -> (Stream, UnixStream) {
    ...
    let stream = Stream::connect_unix(&path, UnixConnectOptions { retry }).unwrap();
...
fn a_not_ready_read_is_retried_until_the_bytes_arrive() {
    let (mut stream, mut far) = connected("retried", Retry::Until(Duration::from_secs(5)));
...
fn a_read_that_never_becomes_ready_gives_up_with_a_timeout() {
    let (mut stream, _far) = connected("timeout", Retry::Until(Duration::from_millis(50)));
...
fn never_hands_a_not_ready_read_straight_back() {
    let (mut stream, _far) = connected("never", Retry::Never);
...
fn a_broken_stream_is_reported_on_the_spot() {
    let (mut stream, far) = connected("broken", Retry::Until(Duration::from_secs(60)));
    drop(far);
